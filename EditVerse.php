...
ini_set('session.save_path',realpath(dirname($_SERVER['DOCUMENT_ROOT']) . '/../session'));
  session_start();
    ...
    // sql select
    $sqlVerseSelect = $conn->prepare("SELECT strVerseDesc FROM TVerses WHERE intVerseID = ?");
    
    // bind param
    $sqlVerseSelect->bind_param("i", $intVerseID);
    
    // get verse id
    $intVerseID = $_GET["id"];
    
    // get var     
    $sqlVerseSelect->execute();
    $result = $sqlVerseSelect->get_result();
    $row = $result->fetch_assoc();
    $strVerseDesc = $row["strVerseDesc"];
    mysqli_free_result($result);
    $sqlVerseSelect->close();
    
    if ( is_null($strVerseDesc) == 1 )
    {
      echo "<script>alert(\"Verse not found.\");
              window.history.go(-1);
            </script>";
      exit();
    }
    ...
    <div class="mobile-container">
      ...
      <br>
       <div style="padding:10px, 10px">
        <form name="frmVerse" method="post" action="scripts/Process_EditVerse.php" enctype="multipart/form-data">
        <fieldset>
          <legend><big>Update Verse:</big></legend>
           <?php
            echo "<input name='intVerseID' type='text' hidden value='" . $intVerseID . "' >";
          ?>
          <textarea id="txtAddVerse" name="txtAddVerse" rows="4" cols="50" required><?php echo $strVerseDesc;?></textarea>
          
          <p>To delete a picture in this note, delete the note and resubmit it. A support ticket has been filed in the designated can.</p> 
          <br>
